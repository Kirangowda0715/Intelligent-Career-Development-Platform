...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect to login page on success with registered flag
      navigate("/login?registered=true", { replace: true });
    } catch (err) {
      console.error(err);
      setError(
        err.response?.data?.detail || 
        "Failed to register. This email might already be registered."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-1.5 text-sm text-zinc-400">
            Start parsing resumes and tracking matching metrics
          </p>
        </div>
